...
from cm.utils.utilities import synchronized, daemonize
...
def convert_html(input, format_name, images = None):
    ...
    try:
        ...
        infile.write(fix_content(input))
        infile.close()
...
def convert(input, format_name, unicode = False):
    ...
    try:
        ...
        for image_name in image_names:
              img_content = open(outdir_name + "/" + image_name).read()
              img_res[image_name] = img_content
        if unicode:
