...
-- using in co-ment < 0.98 using Postgresql 8?2

-- BEGIN /usr/share/postgresql/8.2/contrib/tsearch2.sql
------------------------
-- This code is from tsearch2 initialisation / see bellow for custom co-ment code
-- only BEGIN / END and comments (containing ';') where removed 
------------------------
...
-- END /usr/share/postgresql/8.2/contrib/tsearch2.sql
--ALTER TABLE cm_textversion DROP COLUMN idxFTI
ALTER TABLE cm_textversion ADD COLUMN idxFTI tsvector;

UPDATE cm_textversion SET idxFTI=to_tsvector('simple',coalesce(content,'') ||' '|| coalesce(title,''));
-- VACUUM FULL ANALYZE

--DROP INDEX idxFTI_idx
CREATE INDEX idxFTI_idx ON cm_textversion USING gist(idxFTI);
-- VACUUM FULL ANALYZE

UPDATE pg_ts_cfg SET locale=(SELECT setting FROM pg_settings WHERE name LIKE 'lc_collate') WHERE ts_name='simple' AND prs_name='default';

--DROP TRIGGER tsvectorupdate_cm_textversion ON cm_textversion
CREATE TRIGGER tsvectorupdate_cm_textversion BEFORE UPDATE OR INSERT ON cm_textversion
            FOR EACH ROW EXECUTE PROCEDURE
                tsearch2(idxFTI, content, title);
